...
class MGraph__Graph(Type_Safe):
    ...
    def edges(self) -> List[MGraph__Edge]:
        return [self.mgraph_edge(edge=edge) for edge in self.graph.edges()]

    def mgraph_edge(self, edge: Model__MGraph__Edge) -> MGraph__Edge:
        #edge_domain_type = self.graph.default_types.node_domain_type
        edge_domain_type = MGraph__Edge
        return edge_domain_type(edge=edge, graph=self.graph)

    def mgraph_node(self, node: Model__MGraph__Node) -> MGraph__Edge:
        #node_domain_type = self.graph.default_types.node_domain_type
        node_domain_type = MGraph__Node
        return node_domain_type(node=node, graph=self.graph)
    ...
    def nodes(self) -> List[MGraph__Node]:
        return [self.mgraph_node(node=node) for node in self.graph.nodes()]
