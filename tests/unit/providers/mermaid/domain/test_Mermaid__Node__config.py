...
from mgraph_db.providers.mermaid.utils.Test_Data__Mermaid             import Test_Data_Mermaid


class test_Mermaid__Node__config(TestCase):     # todo: refactor these tests to the correct location since most are related to the node's .node_data
    ...
    def test_markdown(self):
        assert self.first_node.node_data.markdown is False
        self.first_node.markdown(True)
        assert self.first_node.node_data.markdown is True
        self.first_node.markdown(False)
        assert self.first_node.node_data.markdown is False

    def test_wrap_with_quotes(self):
        assert self.first_node.node_data.wrap_with_quotes is True
        self.first_node.wrap_with_quotes(False)
        assert self.first_node.node_data.wrap_with_quotes is False
        self.first_node.wrap_with_quotes(True)
        assert self.first_node.node_data.wrap_with_quotes is True

    def test_show_label(self):
        assert self.first_node.node_data.show_label is True
        self.first_node.show_label(False)
        assert self.first_node.node_data.show_label is False
    ...
    def test_shape_methods(self):
        ...
        assert self.first_node.node_data.node_shape == Schema__Mermaid__Node__Shape.circle
        ...
        for method_name, expected_shape in shape_methods:
            ...
            assert self.first_node.node_data.node_shape == expected_shape

    def test_shape_invalid(self):
        # Test with invalid shape name
        self.first_node.shape('invalid_shape')
        assert self.first_node.node_data.node_shape == Schema__Mermaid__Node__Shape.default
